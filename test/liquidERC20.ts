...
describe("TestLiquidERC20", () => {
  ...
  beforeEach("deploy", async () => {
    ...
    if (await deployed.getDeployedCode()) {
      console.log("Using already deployed contract: ", DEPLOYED_CONTRACT);
      token = deployed as unknown as LiquidInfrastructureERC20;
      let erc20Addresses = await token.getDistributableERC20s();
      let i = -1;
      for (let erc20 of erc20Addresses) {
        i += 1;
        let erc20Factory = await ethers.getContractFactory("TestERC20A");
        erc20s[i] = new ethers.Contract(
          erc20,
  ...
  it("Revenue Claim with Accounting", async () => {
    ...
    let revenueByHolder = stakeByHolder.map((v) =>
      totalRevenueByERC20.map((w) => Math.floor((v * w) / totalStake))
    );
  ...
  it("Add Distributable ERC20", async () => {
    ...
    let revenueByHolder = stakeByHolder.map((v) =>
      totalRevenueByERC20.map((w) =>
        BigNumber(Math.floor((v * w) / totalStake))
      )
    );
    await stakeFromHolders(signers, token);
    ...
    for (let i = 0; i < nfts.length; i++) {
      ...
      await token.setManagedNFTThresholds(
  ...
  it("BigNumberRevenue Claim z with Accounting", async () => {
    ...
    let revenueByHolder = stakeByHolder.map((v) =>
      totalRevenueByERC20.map((w) => v.times(w).div(totalStake).integerValue())
    );
    await stakeFromHolders(signers, token);
    ...
    for (let i = 0; i < signers.length; i++) {
      for (let j = 0; j < erc20s.length; j++) {
        let actualIncrease = BigNumber(balanceBySignerAfter[i][j]).minus(
          balanceBySignerBefore[i][j]
        );
        let acceptableIncrease = revenueByHolder[i][j].times(
          BigNumber(1).minus(acceptableError)
        );
  ...
  it("Many Stakers BigNumber Revenue Claim", async () => {
    ...
    let stakeByHolder = holders.map((v) =>
      BigNumber(randi(100000000) + 1).times(oneEth)
    );
    await mintToHolders(deployer, token, holderAddresses, stakeByHolder);
  ...
  it("BigNumber Three Phase Staking", async () => {
    ...
    let firstWaveTotalStake = stakeByHolder.reduce(
      (a, b, i) => a.plus(i < 50 ? b : 0),
      BigNumber(0)
    );
    let secondWaveTotalStake = stakeByHolder.reduce(
      (a, b, i) => a.plus(i < 100 && i > 49 ? b : 0),
      BigNumber(0)
    );
    let thirdWaveTotalStake = stakeByHolder.reduce(
      (a, b, i) => a.plus(i > 99 ? b : 0),
      BigNumber(0)
    );
    ...
    for (let i = 100; i < 150; i++) {
      let totalRelevantStake = firstWaveTotalStake
        .plus(secondWaveTotalStake)
        .plus(thirdWaveTotalStake);
      let expectedRevenueThird = revenuePerNFTByERC20.map((v) =>
        stakeByHolder[i].times(v).div(totalRelevantStake)
      );
      let acceptableRevenueThird = expectedRevenueThird.map((v) =>
        v.times(1 - acceptableError)
      );
      let actualRevenueThird = balanceBySignerAfter[i].map((v, j) =>
        BigNumber(v).minus(balanceBySignerBefore[i][j])
      );
      actualRevenueThird.map(
        (v, j) => expect(v.gte(acceptableRevenueThird[j])).to.be.true
      );
    }
    ...
    for (let i = 50; i < 100; i++) {
      ...
      let expectedRevenueSecond = revenuePerNFTByERC20.map((v) =>
        stakeByHolder[i].times(v).div(totalRelevantStake)
      );

      let expectedRevenue = expectedRevenueSecond.map((v, j) =>
        v.plus(expectedRevenueThird[j])
      );

      let acceptableRevenue = expectedRevenue.map((v) =>
        v.times(1 - acceptableError)
      );
      let actualRevenue = balanceBySignerAfter[i].map((v, j) =>
        BigNumber(v).minus(balanceBySignerBefore[i][j])
      );
      actualRevenue.map(
        (v, j) => expect(v.gte(acceptableRevenue[j])).to.be.true
      );
    }
    ...
    for (let i = 0; i < 50; i++) {
      ...
      let expectedRevenueFirst = revenuePerNFTByERC20.map((v) =>
        stakeByHolder[i].times(v).div(totalRelevantStake)
      );

      let expectedRevenue = expectedRevenueFirst.map((v, j) =>
        v.plus(expectedRevenueSecond[j]).plus(expectedRevenueThird[j])
      );
  ...
  it("BigNumber Agent Staking", async () => {
    ...
    withdrawalData.push({
      totalStake: BigNumber(0),
      stakers: new Array<StakerData>(initialStakers),
      nfts: new Array<NFTData>(initialManagedNFTs),
      nftRemovals: new Array<NFTData>(),
    });
    await Promise.all(
      nfts.slice(0, initialManagedNFTs).map(async (n, i) => {
        await n.setApprovalForAll(await token.getAddress(), true);
        await token.addManagedNFT(await n.getAddress());
        withdrawalData[0].nfts[i] = {
          nft: n,
          balances: erc20s.map((v) => BigNumber(0)),
          thresholds: erc20s.map((v) => BigNumber(0)),
        };
      })
    );
    let startingStakers = holders.slice(0, initialStakers);
    let stakeAllocation = startingStakers.map((h) =>
      BigNumber(randi(1000000)).times(oneEth)
    );
    ...
    for (let h = 0; h < holders.length; h++) {
      ...
      for (let e = 0; e < erc20s.length; e++) {
        balances[e] = BigNumber(
          (await erc20s[e].balanceOf(holders[h].address)).toString()
        );
      }
    ...
    while (Date.now() - start < three_minutes) {
      ...
      for (let s = 0; s < stakerActionsPerWithdrawal; s++) {
        ...
        if (action < 10) {
          ...
          let unstaker =
            withdrawalData[w].stakers[withdrawalData[w].stakers.length - 1];
          ...
          let claimer =
            withdrawalData[w].stakers[randi(withdrawalData[w].stakers.length)];
      ...
      for (let o = 0; o < ownerActionsPerWithdrawal; o++) {
        ...
        if (action < 20 && unmanagedNFTs.length > 0) {
          ...
          await token.releaseManagedNFT(
            await nft.nft.getAddress(),
            deployer.address
          );
      ...
      let funding = withdrawalData[w].nfts.map((v) =>
        BigNumber(randi(1000000) + 1).times(oneEth)
      );
      let nftAddresses = await Promise.all(
        withdrawalData[w].nfts.map(async (v) => await v.nft.getAddress())
      );
      fundNFTs(deployer, nftAddresses, erc20s as TestERC20A[], funding);

      for (let i = 0; i < withdrawalData[w].nfts.length; i++) {
        withdrawalData[w].nfts[i].balances = await Promise.all(
          erc20s.map(async (v) =>
            BigNumber(await v.balanceOf(await nftAddresses[i]).toString())
          )
        );
        let [_, thresholds] = await withdrawalData[w].nfts[
          i
        ].nft.getThresholds();
        withdrawalData[w].nfts[i].thresholds = thresholds.map((v) =>
      ...
      withdrawalData.push({
        totalStake: withdrawalData[w].totalStake,
        stakers: withdrawalData[w].stakers,
        nfts: withdrawalData[w].nfts,
        nftRemovals: [],
      });
    }

    let holderBalancesAfter: BigNumber[][] = new Array<BigNumber[]>(
      holders.length
    );
    for (let h = 0; h < holders.length; h++) {
      let balances: BigNumber[] = new Array<BigNumber>(erc20s.length);
    ...
    holderAddresses.map(
      (h) => (expectedHolderRevenue[h] = erc20s.map((e) => BigNumber(0)))
    );
    for (let w = 0; w < withdrawalData.length; w++) {
      ...
      let revenuePerStake = expectedTotalRevenue.map((v) =>
        v.div(withdrawalData[w].totalStake)
      );
      ...
      for (let staker of withdrawalData[w].stakers) {
        ...
        for (let i = 0; i < expectedRevenue.length; i++) {
          expectedHolderRevenue[staker.signer.address][i] =
            expectedHolderRevenue[staker.signer.address][i].plus(
              expectedRevenue[i]
            );
        }
      }
        ...
        if (!actualIncrease.gte(acceptableIncrease)) {
          console.log(
            "Holder: " +
              holders[h].address +
              " ERC20: " +
              (await erc20s[e].getAddress()) +
              " Expected: " +
              expectedRevenue.toFixed() +
              " Actual: " +
              actualIncrease.toFixed()
          );
        }
        expect(actualIncrease.gte(acceptableIncrease)).to.be.true;
      }
